...
const handleConfirmLogout = async () => {
    try {
      // 1. Panggil Server Action untuk hapus Cookie
      await performLogout();
      
      // 2. Bersihkan state Redux di Client
      dispatch(setUser(null));
      
      // 3. Tutup Modal & Sidebar
      setIsLogoutModalOpen(false);
      setIsMobileOpen(false);

      // 4. Paksa Refresh & Redirect ke Home
      // router.refresh() penting agar Middleware mendeteksi cookie sudah hilang
      router.refresh(); 
      router.push('/');
...
{/* Sidebar Mobile */}
      <MobileSidebar 
        ...
        user={userData}
        onLogoutClick={() => setIsLogoutModalOpen(true)} // Trigger Modal
      />
      ...
      {isOpen && (
        <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative bg-white dark:bg-gray-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden p-6 text-center"
          >
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 text-sm">
              Apakah Anda yakin ingin keluar dari akun Admin? Anda harus login kembali untuk mengakses dashboard ini.
            </p>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={onConfirm}
                className="flex-1 py-2.5 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 shadow-lg shadow-red-600/20 transition-colors"
              >
                Ya, Keluar
              </button>
            </div>
