...
import { triggerAuthRefresh } from '../../../store/slice/authslice';
...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (isRedirecting) {
      // SKENARIO A: Data User SUDAH MASUK di Redux
      if (user) {
        // 1. Refresh Server Components (agar Middleware/Server tau kita sudah login)
        router.refresh();
        // 2. Pindah Halaman (State Redux TETAP AMAN, tidak hilang)
        router.push('/');
        return;
      }
      ...
      timeoutId = setTimeout(() => {
        if (!user) {
          console.warn("⚠️ Timeout! Data belum masuk. Mencoba fetch ulang...");

          // Usaha terakhir: Fetch lagi
          dispatch(triggerAuthRefresh());

          // Jika masih bandel, paksa pindah (Fallback)
          setTimeout(() => {
  ...
  }, [isRedirecting, user, dispatch, router]);
    ...
    try {
      if (isLoginView) {
        ...
        if (error) {
          ...
          // TRIGGER GLOBAL AUTH REFRESH (Agar Redux diisi data terbaru)
          dispatch(triggerAuthRefresh());
        ...
        // Untuk register, selalu matikan loading di akhir (baik sukses/gagal)
        setIsLoading(false);
      }
    } catch (error) {
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <motion.div variants={itemVariants} className="pt-2">
                    ...
                    >
                      {(isLoading || isRedirecting) ? <Loader2 className="w-5 h-5 animate-spin" /> : (isLoginView ? "Sign In" : "Create Account")}
                      {!(isLoading || isRedirecting) && <ArrowRight size={18} />}
                    </button>
                  </motion.div>
                </form>
